...
public class BotErrorHandlerManager {
	...
	public BotErrorHandlerManager(BotErrorHandlerManagerConfig config, CacheManager cacheManager, BotService botService,
			MessageSource messageResource) {
		this.config = config;
	...
	public void stop() {
		...
		if (botErrorHandler != null)
			botErrorHandler.stop();
