...
public class InfinispanCacheManagerProxy extends CacheManagerProxy {
	...
	public void initialize() throws Exception {
		...
		for (CacheConfig distributeCacheConfig : distributeCacheConfigs) {
			InfinispanCacheProxy<String, Trace> icp = createCache(diskPath, TransactionMode.NON_TRANSACTIONAL,
					distributeCacheConfig);
		...
		for (CacheConfig botCacheConfig : botCacheConfigs) {
			InfinispanCacheProxy<String, StateEvent> cache = createCache(diskPath, TransactionMode.NON_TRANSACTIONAL,
					botCacheConfig);
			botCaches.add(cache);
		}
		...
		if (testCacheConfig != null)
			testCache = createCache(diskPath, TransactionMode.NON_TRANSACTIONAL, testCacheConfig);
		...
		if (interfaceCacheConfig != null)
			interfaceCache = createCache(diskPath, TransactionMode.NON_TRANSACTIONAL, interfaceCacheConfig);
		...
		if (unmatchCacheConfig != null)
			unmatchCache = createCache(diskPath, TransactionMode.NON_TRANSACTIONAL, unmatchCacheConfig);
		...
		if (systemErrorCacheConfig != null)
			systemErrorCache = createCache(diskPath, TransactionMode.NON_TRANSACTIONAL, systemErrorCacheConfig);
	 ...
	 * @return
	 */
	private <K, V> InfinispanCacheProxy<K, V> createCache(String diskPath, TransactionMode mode,
