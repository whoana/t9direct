...
public class Finisher implements Runnable {
	...
	public Finisher(
		...
		cleaner = useWaitForCleaning
				? new DefaultCleaner(waitForCleaning, waitForFinishedCleaning, backupCache, finCache, routingCache)
				: new NoWaitingCleaner(waitForCleaning, backupCache, finCache, routingCache);
	...
	public void runAsap() {
		...
		while (true) {
			try {
				...
				// boolean doCleaning = System.currentTimeMillis() - beforeCleaningEndTime >=
				// waitForCleaning;
