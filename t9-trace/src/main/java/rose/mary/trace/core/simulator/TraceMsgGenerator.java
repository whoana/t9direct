...
public class TraceMsgGenerator {
	...
	public void initialize() throws Exception {
		...
		if (interfaceList.size() > 0)
			interfaceIndex = new IntCounter(0, interfaceList.size() - 1, 1);
	...
	public void generate(String integrationId) throws Throwable {
		try {
			int putCount = 0;
			String interfaceId = null;
			String status = "00";
			for (int i = 0; i < generateCount; i++) {
				interfaceId = integrationId == null ? ramdomInterfaceId("INTF", i + 1) : integrationId;
				status = tmc.createStatus(i + 1);
				List<MTEHeader> headers = tmc.create(interfaceId, status);
				for (MTEHeader header : headers) {

					String key = header.getaIntfId() + "@" + header.getaDate() + header.getaTime() + "@"
							+ header.getbHostId() + "@" + header.getcProcessId();
	...
	public void generateNodeMsg(Map<String, String> params) throws Throwable {
		try {
			...
			String processHostId = params.get("processHostId");
			String processOsType = params.get("processOsType");
			String processOsVersion = params.get("processOsVersion");
			String processId = params.get("processId");
			String processType = params.get("processType");
			String processMode = params.get("processMode");
			String hubCnt = params.get("hubCnt");
			String spokeCnt = params.get("spokeCnt");
			String recvSpokeCnt = params.get("recvSpokeCnt");
			String hopCnt = params.get("hopCnt");
			String appType = params.get("appType");
			String recordCnt = params.get("recordCnt");
			String recordSize = params.get("recordSize");
			String dataSize = params.get("dataSize");
			String dataCompressYn = params.get("dataCompressYn");
			String errorCode = params.get("errorCode");
			String errorMessage = params.get("errorMessage");

			MTEHeader header = tmc.create(interfaceId, date, time, groupId, hostId, status, nodeName, processHostId,
					processOsType, processOsVersion, processId, processType, processMode, hubCnt, spokeCnt,
					recvSpokeCnt, hopCnt, appType, recordCnt, recordSize, dataSize, dataCompressYn, errorCode,
					errorMessage);
			tmh.put(header, data);
			tmh.commit();
		} catch (Throwable t) {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		try {
			...
			TraceMsgGenerator tmg = new TraceMsgGenerator(hostname, port, qmgrName, userId, password, channelName,
					queueName, module, generateCount, commitCount, data, tmc, null);
	...
	private static void showHelp() {
		...
		System.out.println(
				"\tTraceMsgGenerator 는 모코엠시스 EAI ASIS 트레킹 제품을 테스트를 위해 RFH2 헤더정의를 포함한 트레킹 메시지를 큐에 발생시키는 테스트 프로그램입니다.");
		System.out.println("How to use:");
		System.out.println(
				"\tjava TraceMsgGenerator -po {port} -ho {hostname} -qm {qmgrName} -ch {channelName} -qu {queueName} -mo {module} -ge {generateCount} -co {commitCount} -tr {traceMsgCreator} -da {data}");
		...
		System.out.println(
				"\tjava TraceMsgGenerator -po 58414 -ho 10.1.1.168 -qm TESTQM08 -ch SYSTEM.DEF.SVRCONN -qu TEST.LQ -mo w -ge 1000 -co 100 -tr rose.mary.trace.test.DefaultTraceMsgCreator -da 1234567890");
	...
	private static boolean checkParams(Map<String, String> params) {
		...
		if (ok) {
			try {
				Integer.parseInt((String) params.get("-po"));
			} catch (NumberFormatException e) {
				ok = false;
				System.out.println("-po 파라미터는 숫자여야 합니다.");
			}
			try {
				Integer.parseInt((String) params.get("-ge"));
			} catch (NumberFormatException e) {
				ok = false;
				System.out.println("-ge 파라미터는 숫자여야 합니다.");
			}
			try {
				Integer.parseInt((String) params.get("-co"));
			} catch (NumberFormatException e) {
				ok = false;
