...
public class MQMessageParser extends Parser {
	...
	public Trace parse(Object traceObject) throws Exception {
		...
		if (msg.getTotalMessageLength() == 0)
			throw new ZeroLengthMessageException();
		...
		if (CMQC.MQFMT_RF_HEADER_2.equals(msg.format)) {
			...
			os = parseStringValue(hostInfo, MTEStruct.b_os_type, false)
					+ parseStringValue(hostInfo, MTEStruct.b_os_version, false);
			// -------------------------------------------------------------------
			...
			processDate = parseStringValue(processInfo, MTEStruct.c_date, true)
					+ parseStringValue(processInfo, MTEStruct.c_time, true);
			...
			trace.setId(Util.join(trace.getIntegrationId(), idSeperator, trace.getDate(), idSeperator, hostId,
					idSeperator, processId));
