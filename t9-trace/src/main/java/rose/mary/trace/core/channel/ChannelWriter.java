...
public class ChannelWriter implements Runnable {
	...
	public void run() {
		...
		logger.info(Util.join("start channel writer:", this.getClass().getName(),
		...
		while (Thread.currentThread() == thread && !isShutdown) {
			...
			if ((channel.uncommitedSize() % channel.getCommitCount() == 0
					|| (System.currentTimeMillis() - commitLapse >= channel.getMaxCommitWait()))) {
				// if(channel.uncommitedSize() > 0 && (channel.uncommitedSize() %
				// channel.getCommitCount() == 0 || (System.currentTimeMillis() - commitLapse >=
				// channel.getMaxCommitWait()))) {
				try {
					channel.commit();
		...
		isShutdown = true;
		logger.info(Util.join("stop channel writer:", this.getClass().getName(),
				"[" + Thread.currentThread().getName() + "]"));
